    ...
    return (
      <div className="flex items-center gap-1.5">
        <button
          onClick={onConnect}
          disabled={busy}
          className="inline-flex items-center gap-1.5 rounded-full border border-slate-300 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 shadow-sm transition-colors hover:border-brand hover:text-brand disabled:opacity-50"
        >
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        ...
        className="inline-flex items-center gap-1.5 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs hover:border-emerald-300"
      >
        ...
        {balance !== null && (
          <span className="hidden font-mono text-emerald-700 sm:inline">· {balance} USDC</span>
        )}
      </button>

      {menuOpen && (
        <div className="absolute right-0 top-full z-50 mt-2 w-64 overflow-hidden rounded-xl border border-slate-200 bg-white p-1 shadow-xl">
          <div className="px-3 py-2 border-b border-slate-100">
            <p className="font-mono text-[9px] uppercase tracking-widest text-slate-400">Connected wallet</p>
            <p className="mt-0.5 truncate font-mono text-[11px] text-slate-900" title={address}>
              {address}
            </p>
            {balance !== null && (
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs text-slate-700 hover:bg-slate-100"
