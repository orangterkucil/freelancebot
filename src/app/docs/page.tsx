        ...
        <main className="py-8 lg:py-12">
          <div className="prose-doc max-w-[68ch]">
            ...
            <Section id="fees" title="Fees & sustainability" emoji="🪙">
              <p>FreelanceBot charges a <strong>flat 1% protocol fee</strong> on each released escrow (on-chain: <code>agentFeeBps = 100</code>). The freelancer receives 99% of the agreed amount; the 1% is what keeps the service running.</p>
              <p className="rounded-lg border border-amber-300 bg-amber-50 px-3 py-2.5 text-[13px] leading-relaxed text-amber-900 dark:border-amber-500/40 dark:bg-amber-500/10 dark:text-amber-200">
                <strong>1% is enforced by the contract, not by our word — in the repo.</strong> <code>MAX_FEE_BPS = 100</code> means the owner can lower the fee but the contract rejects any attempt to raise it past 1%, and the rate is locked onto each order when it is funded, so you are charged what you agreed to. <strong>The instance deployed today predates both changes:</strong> its ceiling is 10% and it reads the fee at release. We chose not to redeploy days before a submission deadline — a new address means re-verifying source and migrating live escrows. Until it ships, every fee change emits <code>AgentFeeUpdated</code> on-chain, so a raise would be public.
              </p>
