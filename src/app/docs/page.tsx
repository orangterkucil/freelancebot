          ...
          <div className="prose-doc max-w-3xl">
            ...
            <Section id="fees" title="Fees & sustainability" emoji="🪙">
              ...
              <p className="rounded-lg border border-amber-300 bg-amber-50 px-3 py-2.5 text-[13px] leading-relaxed text-amber-900 dark:border-amber-500/40 dark:bg-amber-500/10 dark:text-amber-200">
                <strong>Be clear about what 1% is.</strong> It is the value we chose, not a limit the contract enforces — <code>setAgentFee</code> reverts only above 10%, the owner can change it at any time, and on the <em>currently deployed</em> contract <code>release()</code> uses whatever the fee is at the moment of release rather than the one in force when the escrow was funded. Every change emits <code>AgentFeeUpdated</code> on-chain, so a raise is public. The fix — locking the rate into each order at funding — is written and tested in the repo, and ships with the next contract deploy.
              </p>
